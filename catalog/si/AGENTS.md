...
Licensed [License](https://rkg.gov.si/vstop/).
...
| file | what it is |
| --- | --- |
| `si_fields.parquet` | Field boundary polygons |
...
- Field coverage is uneven: the median chip is 18.8% mapped field while the bottom 5% sit at or below 0.6%. Filter on `field_coverage_pct` when sparse chips would skew an evaluation.
...
```sql
SELECT id, field_coverage_pct FROM read_parquet('si_chips.parquet') ORDER BY field_coverage_pct DESC LIMIT 5;
-- result: ftw-33TWM5440 | 94.56
-- result: ftw-33TWM9662 | 90.09
-- result: ftw-33TWM5436 | 88.65
-- result: ... 2 more rows
```
...
```sql
SELECT hcat_dominant_name_en AS crop, count(*) AS chips FROM read_parquet('si_chips.parquet') WHERE hcat_dominant_code IS NOT NULL GROUP BY 1 ORDER BY 2 DESC LIMIT 10;
-- result: Permanent grassland | 4384
-- result: Grain maize | 335
-- result: Common grape vine | 129
-- result: ... 7 more rows
```
...
```sql
SELECT count(*) AS fields FROM read_parquet('si_fields.parquet') f, (SELECT geometry FROM read_parquet('si_chips.parquet') LIMIT 1) c WHERE ST_Intersects(f.geometry, c.geometry);
-- result: 83
```

## Related collections

The field boundaries here come from [Source field boundary collection](https://data.source.coop/ftw/harmonized-field-data/si/collection.json); consult it for the original attributes, licensing terms and update cadence.
